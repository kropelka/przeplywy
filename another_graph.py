...
#            res.show_on_graph()
    ...
    def dinic_rez(self, v, fl):
        if v==self.n-1 or fl==0:
            return fl
        for u in self.za[v]:
            if self.dist[v] +1 == self.dist[u] and self.cap[u,v] > self.flow[u,v]:
                self.flow[u,v] += self.dinic_rez(u, min(fl, self.cap[u,v] - self.flow[u,v]))

        zuzyte = set()

        zablokowane = numpy.zeros(self.n-1, self.n-1)
        stos = deque()
        #v = 0
        while v!=self.n-1:
            if self.za[v]: #jesli jest jakis wierzcholek z przodu
                v = self.za[v][0] #rusz dalej
                sciezka.append(v)
            else:
                u = sciezka.pop()
                self.przed[v].remove()
...
def losowa_siec_przeplywowa(n, m, max_cap):
    siec = SiecPrzeplywowa(n)
    g = nx.gnm_random_graph(n, m, directed=True)
    for (u,v) in g.edges():
        siec.dodaj_krawedz(u,v)
        siec.ustal_pojemnosc(u,v, random.randint(0,max_cap))


    siec = SiecPrzeplywowa(n)
    for u in range(0, n):
        ilekr = random(0, n-2)
        wierzch = random.sample(x, ilekr)
        for v in wierzch:
            if v!=u:
                siec.dodaj_krawedz(u,v)
